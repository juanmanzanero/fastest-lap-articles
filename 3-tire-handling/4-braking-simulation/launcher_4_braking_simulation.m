...
%plot_locking_simulation(run_lock,s(1:n_points_lock_sim+1),vehicle,state_names,algebraic_state_names,control_names,output_names,'1_lock_simulation')
...
for i_frame = 1:numel(time)
    ...
    if ( abs(q_i(1) + 1.0) < 0.1 )
        if ( mod(i_frame,30) >= 15 )
            text(x_rr + 1.6+ 1.8, 1.2,'Locked','FontName','Formula1','HorizontalAlignment','center','backgroundColor',[1,0,0])
        end
    end
    
    if ( abs(q_i(3) + 1.0) < 0.1 )
        if ( mod(i_frame,30) >= 15 )
            text(x_rr, 1.2,'Locked','FontName','Formula1','HorizontalAlignment','center','backgroundColor',[1,0,0])
        end
    end
    ...
    plot(q_i(1),pacejka_model_longitudinal(-qa_i(1)*660*9.81,q_i(1),tire_data),'o','MarkerSize',12,'Color',[1,0,0],'LineWidth',2);
    plot(q_i(1),pacejka_model_longitudinal(-qa_i(1)*660*9.81,q_i(1),tire_data),'.','MarkerSize',12,'Color',[1,0,0],'LineWidth',2);
    
    if ( abs(q_i(1) + 1.0) < 0.1 )
        if ( mod(i_frame,30) >= 15 )
            text(-0.8, 500.0,'Locked','FontName','Formula1','HorizontalAlignment','center','backgroundColor',[1,0,0],'FontSize',20)
        end
    end
